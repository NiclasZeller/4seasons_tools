...
for d in *; do
    if [ -d $d ]; then
        echo "$d"
	if [ ! -f $d/odometry_resutls/orb_slam_stereo_inertial.txt ]; then
	    $ORBSLAM_DIR/Examples/4seasons/4seasons_runner \
                $BOW_VOCABULARY \
                $ORBSLAM_CALIB_FILE \
                $d \
		"with_imu"
	    if [ ! -d $d/odometry_resutls ]; then
            ...
            mv CameraTrajectory.txt $d/odometry_resutls/orb_slam_stereo_inertial.txt
        fi
    fi
done
